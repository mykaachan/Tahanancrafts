...
export default function EditAddress() {
  ...
  function onPHChange(loc) {
    setForm({ ...form, region: loc.region, province: loc.province, city: loc.city, barangay: loc.barangay });
  }

  async function save() {
    const res = await fetch(`${process.env.REACT_APP_API_BASE || "https://tahanancrafts.onrender.com"}/api/users/shipping-address/update/${id}/`, {
      method: "PUT",
  ...
  async function setDefault() {
    const res = await fetch(`${process.env.REACT_APP_API_BASE || "https://tahanancrafts.onrender.com"}/api/users/shipping-address/set-default/${id}/`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ user_id: userId }),
    });
    if (res.ok) { alert("Set as default"); navigate("/select-address"); }
  }

  if (!form) return <HeaderFooter><div className="loading">Loading…</div></HeaderFooter>;
        ...
        <div className="address-page-inner small">
          ...
          <div className="form-grid">
            ...
            <div className="picker-row">
              <input name="region_display" value={`${form.province || ""}${form.city ? ", " + form.city : ""}${form.barangay ? ", " + form.barangay : ""}`} readOnly />
            ...
            <div className="toggle-row">
              <label className="switch">
                <input type="checkbox" name="is_default" checked={!!form.is_default} onChange={(e)=> setForm({...form, is_default: e.target.checked})} />
          ...
          <div className="form-actions">
            ...
            <button className="btn-danger" onClick={remove}>Delete Address</button>
            <button className="btn-secondary" onClick={setDefault}>Set as Default</button>
        ...
        {modalOpen && (
          <div className="modal-backdrop" onClick={() => setModalOpen(false)}>
            <div className="modal" onClick={(e) => e.stopPropagation()}>
              ...
              <AddressDropdownPH onChange={onPHChange} initial={{
                region: form.region, regionCode: "", province: form.province, provinceCode: "", city: form.city, cityCode: "", barangay: form.barangay
              }} />
